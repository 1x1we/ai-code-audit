...
![规则版本](https://img.shields.io/badge/规则版本-v8.10-blue) ![脚本版本](https://img.shields.io/badge/脚本版本-v8.10-blue) ![Python](https://img.shields.io/badge/python-3.11%2B-green) ![License](https://img.shields.io/badge/license-MIT-green)
...
| **发布门禁** | `BLOCK` / `CONDITIONAL` / `PASS` 三态硬决策 | ✅ |
| **LLM 三态协同** | CONFIRM / REJECT / SUPPLEMENT，带审计链和人工复核兜底 | ✅ |
...
- 脚本扫描：~1 秒，0 Token，产出 7 个 HIGH 级命令注入 + 3 个 MID 路径穿越 + 若干 low 级问题。
...
│  阶段 2：LLM 智能体三态确认                                    │
│  · CONFIRM  → 确认真实问题                                    │
│  · REJECT   → 判定误报（致命类需 human_override）              │
│  · SUPPLEMENT → 补抓规则外问题（不直接 BLOCK）                  │
│  · 审计链：reasoning + evidence + confidence                   │
└──────────────┬──────────────────────────────────────────────┘
               │  verdicts.json
               ▼
┌─────────────────────────────────────────────────────────────┐
...
| 决策 | 含义 | 触发条件 |
|------|------|----------|
| `BLOCK` 🚫 | 禁止发布 | 致命层 critical / 安全/致命层存在 high / 提交私钥或密钥文件 |
...
| 核心单元测试 | 17 套 | `cd tests && python test_core.py` |
